...
<body>
    ...
    <div class="section">
        <div class="auctionProgress">
            ...
            <div class="tab">
                <a href="auctionProgress"><h4>진행 경매</h4></a>
                <a href="auctionScheduled"><h4 class="active">예정 경매</h4></a>
                <div class="search">
                    <span class="icon">
...
<%--                                        <img src="images/auction1.jpg">--%>
                        ...
                        <div class="bidButton">
                            <a href="auctionDetail?auction_lot=${list.auction_lot}&auctionSchedule_id=${list.auctionSchedule_id}"><h5>경매 예정</h5></a>
                        </div>
                    </div> <!--auction 끝-->
                </c:forEach>
            </div> <!--auctionList 끝-->
...
<script>
    $(document).ready(function () {
        ...
        // 남은 시간을 매초 업데이트
        let auctionStartTime = new Date("${list[0].auctionSchedule_start}").getTime();  // 종료 시간 (ISO 형식으로 변환 가능)

        // 즉시 남은 시간을 한 번 계산하여 표시
        $('.startTime').text("경매 시작 -" + calculateRemainingTime(auctionStartTime));

        // 1초마다 업데이트
        setInterval(function() {
            let remainingTime = calculateRemainingTime(auctionStartTime);
    ...
    function searchAuction() {
        ...
        if (query.length > 0) { // 검색어가 있으면 AJAX 요청
            $.ajax({
                url: '/api/auctionScheduledSearch', // 검색 API 호출
                type: 'GET',
                data: { query: query },
                dataType: 'json',
                success: function (data) {
            ...
            $.ajax({
                url: '/auctionScheduled', // 전체 경매 목록을 반환하는 컨트롤러 호출
                type: 'GET',
